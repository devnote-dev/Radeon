...
function parseAll(str) {
    ...
    return {
        short: short.filter(f => f.startsWith('-')).map(f => f.slice(1)),
        long: long.filter(f => f.startsWith('-')).map(f => f.slice(2))
    }
}
...
function parseWithContext(flags, ctx) {
    ...
    for (const flag of flags) {
        if (flag.includes('=')) {
            ...
            parsed.set(flag, { value: flag, parsed: ctx[flag] !== undefined });
        }
    }

    for (const key of Object.keys(ctx)) {
        if (parsed.has(key)) continue;
        parsed.set(key, { value: null, parsed: ctx[key] === Boolean ? false : null });
    }
